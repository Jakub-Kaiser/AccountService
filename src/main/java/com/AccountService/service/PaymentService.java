...
//            );
            ...
            if (!userRepository.existsByEmailIgnoreCase(email)) {
                throw new UserNotFoundException(String.format("User %s does not exist", email));
            }
            //check if employee-period pair is unique:
            if (paymentRepository
                    .existsByEmployeeAndPeriod(email, period)) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format(
                        "Following payment already exists: User: %s. Period: %s", email, period
                ));
            }
...
//        );
        ...
        PaymentEntity paymentEntity = returnPaymentEntityOrThrow(email, period);
